...
export async function compileSFCTemplate({ code, id, type }: CompileSFCTemplateOptions) {
            ...
            if (node.type === 'JSXElement') {
              if (
                node.openingElement.attributes.some(
                  (attr) =>
                    attr.type !== 'JSXSpreadAttribute' && attr.name.name === 'data-v-inspector-file'
                )
              )
                return

              const insertPosition =
                node.start + parseJSXIdentifier(node.openingElement.name as any).length + 1
              const { line, column } = node.loc.start
