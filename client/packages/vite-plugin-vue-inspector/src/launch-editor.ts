...
function addWorkspaceToArgumentsIfExists(args, workspace) {
  if (workspace) args.unshift(workspace)
...
function guessEditor() {
  ...
  if (process.env.VUE_EDITOR) return shellQuote.parse(process.env.VUE_EDITOR)
  ...
  try {
    if (process.platform === 'darwin') {
      ...
      for (let i = 0; i < processNames.length; i++) {
        ...
        if (output.indexOf(processName) !== -1) return [COMMON_EDITORS_OSX[processName]]
      }
      ...
      const output = child_process
        .execSync('ps x --no-heading -o comm --sort=comm', { stdio: ['pipe', 'pipe', 'ignore'] })
        .toString()
...
export function launchEditor(fileName: string, lineNumber: number, colNumber: number) {
  ...
  if (editor.toLowerCase() === 'none') return
  ...
  _childProcess.on('exit', (errorCode) => {
    ...
    if (errorCode) printInstructions(fileName, `(code ${errorCode})`)
  })
